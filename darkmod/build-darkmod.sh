...
main()
{
	...
	if [[ -f "debian/changelog" ]]; then
		...
		dch -v --force-distribution "${pkgver}+${pkgsuffix}-${upstream_rev}" -M --package "${pkgname}" -D "${DIST}" -u "${urgency}"

	else

		dch --create -v --force-distribution "${pkgver}+${pkgsuffix}-${upstream_rev}" -M --package "${pkgname}" -D "${DIST}" -u "${urgency}"
