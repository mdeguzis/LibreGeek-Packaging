...
usage ( ) {
	...
	echo ""
	echo "$0 is a script designed for downloading and upgrading packages 
used in your Stephenson's Rocket mod. It uses the ${sources} file to when 
updating the list of packages found in${repopkglist} and downloads packages to 
${packagedir}."
	echo ""
	echo "Commands:"
	echo "  update - Create ${repopkglist} with all packages found in the 
repos in ${sources}"
	echo "  upgrade - Download the latest version of all packages found in 
packages to ${packagedir}"
...
update ( ) {
	...
	echo "Updating list of packages found in the repositories listed in 
the ${sources} file"
	while read repo; do
...
url="${repourl}/dists/${reponame}/${area}/binary-${arch}/Packages.gz"
					wget ${url} 
					gunzip -cq Packages.gz| grep 
'^Filename:'|cut -d " " -f 2- | sed "s#^#${repourl}/#g" >> ${repopkglist}
					rm Packages.gz
                        	done
               		done
        	fi
...
upgrade ( ) {
	...
	if [ ! -e ${repopkglist} ]; then
		echo -e "Error: ${repopkglist} doesn't exist, run \"${0} 
update\" first\n"
		usage;
	fi
	...
	for package in ${existingpkgs}; do
		...
		versions=$(grep "/${pkgname}_" ${repopkglist}|grep 
"_${pkgarch}"|cut -f2 -d "_")
		newest=$(echo "${pkgversion} ${versions}"|tr "\ " "\n"|sort 
-rV|head -1)
		if [ "${newest}" != "${pkgversion}" ]; then
			pkgdownloadurl=$(grep "/${pkgname}_" 
${repopkglist}|grep "_${pkgarch}"|grep "${newest}"|head -1)
			downloaddir=$(echo ${pkgdownloadurl}|rev|cut -d"/" 
-f2-4|rev)
...
"\n"|nl|grep ${package}|cut -f 1)/$(echo ${clioptions}|wc -w)"
		...
		matches=$(grep ${packagesn} ${repopkglist}|grep 
"_${arch}\.\|_all\."|sort|uniq)
		if [[ "$(echo ${matches}|wc -w)" == "0" ]];then
			echo -e "\n${package} not found in any of the 
