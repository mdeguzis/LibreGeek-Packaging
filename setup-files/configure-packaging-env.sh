...
setup_mock()
{
	...
	# Setup packaging for RPM building
	# Note: requires Fedora user ID!
	usermod -a -G mock ${USER}
		
	# This must complete, or proceed no further
	if ! fedora-packager-setup; then

		echo -e "\nFailed to setup Fedora packaging for user: $USER. Exiting.\n"
...
setup_system_setup()
{
	...
	if [[ "${OS}" == "SteamOS" || "${OS}" == "Debian" || "${OS}" == "Arch Linux" ]]; then
		...
		# brewmaster
		sudo ln -s "/usr/share/debootstrap/scripts/jessie" "/usr/share/debootstrap/scripts/steamos" 2> /dev/null
...
main()
{
	...
	# Output summary if exit code is ok
	if [ $? -eq 0 ]; then
		show_summary
	else
		echo "Failed to setup system!"
	fi
