...
install_prereqs()
{
	...
	if [[ "$arg1"  == '--build-data' ]]; then
		echo -e "==INFO==\nBuilding both 	# inform user of packages
	cat<<- EOF
	#################################################################
...
()
{
	...
	#################################################
	# Prepare package (	# inform user of packages
	cat<<- EOF
	#################################################################
	If package was built without errors you will see it below.
	If you don't, please check build dependency errors listed above.
...
main
	...
	if [[ -f "debian/changelog" ]]; then
 	...
 	#################################################
	# Build Debian package (	# inform user of packages
	cat<<- EOF
	#################################################################
	If package was built without errors you will see it below.
	If you don't, please check build dependency errors listed above.
...
# start main
main
 | tee "/tmp/${PKGNAME}-build-log-tmp.txt"
